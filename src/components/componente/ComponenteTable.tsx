...
export default function ComponenteTable({
  ...
  // Memoiza os dados para evitar recalcular toda renderização
  const data = useMemo(() => componentes, [componentes])

  // Memoiza a função toggleSelecionado para não recriar em cada render
  const memoToggleSelecionado = useCallback(
    (id: number) => () => toggleSelecionado(id),
    [toggleSelecionado]
  )

  const columns = useMemo<ColumnDef<ComponenteItem>[]>(
    () => [
      {
        accessorKey: 'id',
        header: 'ID',
            ...
            return (
              <input
                ...
                onChange={e => {
                  ...
                  setEditaveis(prev =>
                    prev.map(el =>
                      el.id === id ? { ...el, componente_nome: novoValor } : el
                    )
                  )
                }}
                onBlur={() => {
            ...
            return (
              <input
                ...
                onChange={e => {
                  ...
                  setEditaveis(prev =>
                    prev.map(el =>
                      el.id === id ? { ...el, data_recebimento: novaData } : el
                    )
                  )
                }}
                className="border rounded px-2 py-1 w-full"
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <input
            ...
            className="accent-green-600 w-5 h-5 rounded border-gray-300 focus:ring-2 focus:ring-green-500"
            aria-label={`Selecionar componente ${row.original.id}`}
          />
        ),
        enableSorting: false,
        size: 50,
  ...
  if (componentes.length === 0) {
    return (
      <div className="text-center p-4 text-gray-600">Nenhum componente cadastrado.</div>
    )
  }

  return (
    <>
      {/* Botões Salvar todos / Cancelar aparecem em edição múltipla */}
      {modoEdicaoMultipla && (
      ...
      <div className="overflow-x-auto max-h-[400px] overflow-y-auto border rounded-md shadow-sm">
      ...
      <div className="flex items-center justify-between mt-4 text-sm">
        <div>Total: {componentes.length} componentes</div>
