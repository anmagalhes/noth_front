...
const handleSaveSelectedOrder = async () => {
 ...
 const selectedRows = data.filter(row => rowSelection[row.id]);
  ...
  if (confirmation.isConfirmed) {
    ...
    try {
      // Preparar payload apenas com as linhas selecionadas
      // Preparar dados para o backend
    const payload = selectedRows.map(row => ({
      id_tarefa_original: row.idTarefaOriginal, // ID real do banco
    ...
    <div className="my-4 space-y-4">
      ...
      {selectedCount > 0 && !batchEditMode && (
        ...
        >
         ...
         <button
          onClick={handleSaveSelectedOrder}
          className="flex items-center gap-2 rounded-full bg-green-600 px-6 py-3 font-semibold text-white hover:bg-green-700 transition-colors shadow-md hover:shadow-lg transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
